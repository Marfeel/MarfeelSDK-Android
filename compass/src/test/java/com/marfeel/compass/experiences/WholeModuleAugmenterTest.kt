...
class WholeModuleAugmenterTest {
    ...
    private fun module(name: String, vararg positions: Int): RecirculationModule =
        RecirculationModule(
            name = name,
    ...
    @Test
    fun `eligible first time appends whole-module link`() {
        val result = augmenter.onEligible(listOf(module("mod-a", 0, 1)))
        assertEquals(3, result[0].links.size)
        assertEquals(wholeModule, result[0].links.last())
    }

    @Test
    fun `eligible second time for same module does not re-append`() {
        augmenter.onEligible(listOf(module("mod-a", 0)))
        val second = augmenter.onEligible(listOf(module("mod-a", 0, 1, 2)))
        assertFalse(second[0].links.contains(wholeModule))
    }

    @Test
    fun `eligible appends only to modules not yet seen`() {
        augmenter.onEligible(listOf(module("mod-a", 0)))
        val result = augmenter.onEligible(listOf(module("mod-a", 0), module("mod-b", 0)))
        assertFalse(result[0].links.contains(wholeModule))
        assertTrue(result[1].links.contains(wholeModule))
    }

    @Test
    fun `impression after eligible appends whole-module once`() {
        augmenter.onEligible(listOf(module("mod-a", 0)))
    ...
    @Test
    fun `impression without prior eligible does not append whole-module`() {
    ...
    fun `page url change resets module state`() {
        ...
        val second = augmenter.onEligible(listOf(module("mod-a", 0)))
    ...
    fun `client-provided links are preserved when augmenting`() {
        ...
        val result = augmenter.onEligible(listOf(original))
